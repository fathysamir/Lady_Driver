...
@section('title', 'Dashboard - Careers')
@section('content')
<style>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form id="searchForm" class="search-bar"
                              style="margin-bottom:1%;margin-right:20px;margin-left:0px;"
                              method="post" action="{{ route('careers') }}" enctype="multipart/form-data">
                              @csrf
                            <div style="display:flex;">
                                <h5 class="card-title" style="width: 50%;">Applications - {{ $count }}</h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @if(!empty($careers) && $careers->count())
                                        @php $counter = ($careers->currentPage()-1)*$careers->perPage() +1; @endphp
                                        @foreach($careers as $career)
                                        <tr onclick="window.location='{{ route('view.career', ['id'=>$career->id]+request()->query()) }}';" style="cursor:pointer;">
                                            <td>{{ $counter++ }}</td>
                                            <td>{!! highlight($career->first_name . ' ' . $career->last_name, $search ?? '') !!}</td>
                                            <td>{!! highlight($career->email, $search ?? '') !!}</td>
                                            <td>{!! highlight($career->country_code . ' ' . $career->phone, $search ?? '') !!}</td>
                                            <td>{!! highlight($career->position, $search ?? '') !!}</td>
                                            <td>
                                        ...
                                        @endforeach
                                    @else
                                        <tr><td colspan="8">No applicants yet.</td></tr>
                                    @endif
                                </tbody>
                            </table>
                            ...
                            <div style="text-align:center;">
                                {!! $careers->appends(request()->query())->links('pagination::bootstrap-4') !!}
                            </div>
                        </div>
