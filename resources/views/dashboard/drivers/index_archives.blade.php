...
@section('title', 'Dashboard - archived drivers')
@section('content')
    <style>
        .pagination {
            display: inline-flex;
                                    ...
                                    <tbody>
                                        @if (!empty($all_users) && $all_users->count())
                                            ...
                                            @foreach ($all_users as $user)
                                                @php
                                                    $avatarUrl = getFirstMediaUrl($user, $user->avatarCollection) ?? $avatarFallback;
                                                @endphp
                                                <tr>
                                                    <td>{{ $counter++ }}</td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <a onclick='event.stopPropagation(); showConfirmationPopup(
                                                            "{{ url('/admin-dashboard/driver/restore/' . $user->id . '?type=' . $type) }}",
                                                            "{{ $user->name }}",
                                                            "{{ $avatarUrl }}"
                                                        )'>
                                        ...
                                        @else
                                            <tr>
                                                <td>There are no Archived Drivers.</td>
                                            </tr>
                                        @endif
                                    </tbody>
                                </table>
                ...
                <div class="modal-body" style="color:black;">
                    <div class="form-group">
                        <div style="width: 100%; display: flex; justify-content: center;">
                            <img id="deletedUserAvatar"
                                src="{{ asset('dashboard/logo.png') }}"
                    ...
                    <button onclick="hideConfirmationPopup()"
                        ...
                        <span class="bi bi-x" style="font-size: 1rem; color: rgb(255,255,255);"></span> Cancele
                    </button>
                    <button onclick="deleteUser()"
                        style="background-color: rgb(193, 183, 41); color: white; padding: 10px 20px; border: none; cursor: pointer; margin-right: 10px; width:48%; border-radius:10px;">
                        <span class="bi bi-trash" style="font-size: 1rem; color: rgb(255,255,255);"></span> Restore
                    </button>
                </div>
            </div>
        </div>
...
@push('scripts')
    ...
    <script>
        function showConfirmationPopup(deleteUrl, name, imgSrc) {
            ...
            const avatarEl = document.getElementById('deletedUserAvatar');
            avatarEl.src = imgSrc;
            avatarEl.onerror = function () {
                this.onerror = null;
                this.src = '{{ asset('dashboard/user_avatar.png') }}';
            };
