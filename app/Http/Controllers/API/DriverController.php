        ...
        : Car::where('user_id', $user->id)
            ...
            if (!$vehicle) {
                ...
                if ($user->driver_type == 'scooter') {
                    $message = $lang === 'ar'
                        ? 'لم يتم تأكيد بيانات السكوتر الخاص بك بعد. يرجى الانتظار حتى يتم اعتماده.'
                        : 'Your scooter information has not been confirmed yet. Please wait until it is approved.';
                } else {
                    $message = $lang === 'ar'
                        ? 'لم يتم تأكيد بيانات السيارة الخاصة بك بعد. يرجى الانتظار حتى يتم اعتمادها.'
                        : 'Your vehicle information has not been confirmed yet. Please wait until it is approved.';
                }
...
->where(function ($q) {
    $q->where('status', '!=', 'scheduled')
      ->orWhereDoesntHave('offers', function ($oq) {
        ...
        */
        // ===== نهاية الفلتر الحقيقي =====

        // ✅ الشغال دلوقتي: زي الـ Socket بالظبط — بس الـ SQL circle (Haversine) من غير نداء Google
        // بنستخدم client_location_away (اللي اتحسبت في نفس استعلام الـ SQL) كمسافة تقريبية بدل ما ننادي Google
        $trips = $trips->values();
        $trips->transform(function ($trip) {
            $trip->_real_distance_km = $trip->client_location_away ?? null;
            $trip->_real_duration_m  = null; // مفيش مدة حقيقية من غير Google
            return $trip;
        });

        $trips->transform(function ($trip) use ($lat, $lng) {

            // استخدم المسافة التقريبية (خط مستقيم) بدل ما ننادي calculate_distance وندفع تكلفة Google
    ...
    public function driver_current_trip()
    {
        ...
        if ($vehicle) {
            $cacheKey = "driver_current_trip_distance_{$trip->id}";
            $response = Cache::remember($cacheKey, 15, function () use ($vehicle, $trip) {
    ...
    public function update_location_car(Request $request)
{
    $validator = Validator::make($request->all(), [
        ...
        'route.version'      => 'nullable',
        'route.kind'         => 'nullable|string',
    ]);
    ...
    // ================= ROUTE CACHE (client-supplied, free — no Google cost) =================
    $routeKey = $trip ? "trip_{$trip->id}" : "driver_{$user->id}";
    $cacheKey = "route_cache_{$routeKey}";

    $route = Cache::get($cacheKey, []);

    // polyline/kind/version بيتحدّثوا بس لو الكلاينت بعتهم فعليًا
    ...
    // القيم اللي جاية من الكلاينت في الطلب الحالي (لو موجودة) — دي اللي لازم ترجع زي ما هي
    $sentDistance = $request->has('distance') ? $request->distance : null;
    $sentDuration = $request->has('duration') ? $request->duration : null;

    // distance/duration لازم تتحدّث في الكاش مع كل رسالة location فيها القيم دي
    if ($sentDistance !== null) {
        $route['distance'] = $sentDistance;
    }
    if ($sentDuration !== null) {
        $route['duration'] = $sentDuration;
    }
    ...
    // بننادي Google بس لو: فيه trip، والكلاينت مبعتش route/distance/duration خالص، والـ throttle خلص
    $result = null;
    if ($trip && !$sentDistance && !$sentDuration && empty($route['distance'])) {
    ...
    // ================= FINAL PAYLOAD (same shape broadcasted via track_car) =================
    // الأولوية دايمًا للقيمة اللي بعتها الكلاينت في نفس الطلب ده، بعدين الكاش، بعدين نتيجة جوجل
...
$messageAr = $result['message']['ar'] ?? null;
    ...
    // ================= BROADCAST (same recipients as WebSocket: trip owner + driver himself) =================
    $recipientIds = [];

    if ($trip) {
        $recipientIds[] = $trip->user_id;
    }
    ...
    return $this->sendResponse($responseData, $vehicleType . ' location updated successfully', 200);
}
...
public function driver_reached(Request $request)
{
    ...
    public function track_vehicle(Request $request)
    {
        ...
        $cacheKey = "track_vehicle_distance_{$trip->id}";
        $response = Cache::remember($cacheKey, 15, function () use ($request, $trip) {
            return calculate_distance(
                $request->lat,
                $request->lng,
                $trip->start_lat,
