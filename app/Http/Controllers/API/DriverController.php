...
class DriverController extends ApiController
{
    ...
    public function update_location_car(Request $request)
    {
        ...
        if (in_array(auth()->user()->driver_type, ['car', 'comfort_car'])) {
            ...
            $car = Car::where('user_id', auth()->user()->id)->first();
            if (!$car) {
                return $this->sendError(null, "You don't have a car yet", 400);
            }
            ...
            if ($trip) {
                ...
                $durationToPickup      = null;
                $distanceToDestination = null;
                $durationToDestination = null;

                // Driver moving toward passenger — calculate distance to pickup
                if ($trip->status == 'pending') {
                    ...
                    $distanceToPickup = round($response['distance_in_km'], 2);
                    $durationToPickup = intval($response['duration_in_M']);
                }

                // Driver moving toward destination — calculate distance to destination
                if ($trip->status == 'in_progress') {
                    ...
                    if ($destination) {
                        ...
                        $distanceToDestination = round($response['distance_in_km'], 2);
                        $durationToDestination = intval($response['duration_in_M']);
                    }
                }
            ...
            if (!$scooter) {
                return $this->sendError(null, "You don't have a scooter yet", 400);
            }
            if (auth()->user()->is_online == '0') {
                return $this->sendError(null, "You are Offline", 400);
            }
            ...
            if ($trip) {
                ...
                // Driver moving toward passenger
                if ($trip->status == 'pending') {
                    $response = calculate_distance(
                        $request->lat,
                        $request->lng,
                ...
                // Driver moving toward destination
                if ($trip->status == 'in_progress') {
                    $destination = $trip->finalDestination()->orderBy('id', 'desc')->first();
                    if ($destination) {
                        $response = calculate_distance(
