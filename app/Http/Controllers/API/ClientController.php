...
class ClientController extends ApiController
{
    ...
    public function current_trip()
    {
        ...
        $trip = Trip::where('user_id', auth()->user()->id)->whereIn('status', ['created', 'pending', 'in_progress'])->with(['car' => function ($query) {
            ...
            $query->select('id', 'user_id', 'motorcycle_mark_id', 'motorcycle_model_id', 'year', 'lat', 'lng', 'color', 'scooter_plate')->with(['mark:id,en_name,ar_name', 'model:id,en_name,ar_name', 'owner:id,name,country_code,phone,level']);
    ...
    public function calculate_trip_price(Request $request)
{
    ...
    $kilometer_price_short_trip   = floatval(Setting::where('key', 'kilometer_price_' . ($type == 'comfort_car' ? 'comfort' : $type) . '_short_trip')->where('category', $category)->first()->value);
    $kilometer_price_medium_trip  = floatval(Setting::where('key', 'kilometer_price_' . ($type == 'comfort_car' ? 'comfort' : $type) . '_medium_trip')->where('category', $category)->first()->value);
    $kilometer_price_long_trip    = floatval(Setting::where('key', 'kilometer_price_' . ($type == 'comfort_car' ? 'comfort' : $type) . '_long_trip')->where('category', $category)->first()->value);
    $maximum_distance_short_trip  = floatval(Setting::where('key', 'maximum_distance_' . ($type == 'comfort_car' ? 'comfort' : $type) . '_short_trip')->where('category', $category)->first()->value);
    $maximum_distance_medium_trip = floatval(Setting::where('key', 'maximum_distance_' . ($type == 'comfort_car' ? 'comfort' : $type) . '_medium_trip')->where('category', $category)->first()->value);
    $maximum_distance_long_trip   = floatval(Setting::where('key', 'maximum_distance_' . ($type == 'comfort_car' ? 'comfort' : $type) . '_long_trip')->where('category', $category)->first()->value);
    $less_cost_for_trip           = floatval(Setting::where('key', 'less_cost_for_' . ($type == 'comfort_car' ? 'comfort' : $type) . '_trip')->where('category', $category)->first()->value);
    $increase_rate_peak_time      = floatval(Setting::where('key', 'increase_rate_peak_time_' . ($type == 'comfort_car' ? 'comfort' : $type) . '_trip')->where('category', $category)->first()->value);
